...
import { initializePayment, verifyPayment, initializeDirectDebit, ChargeUserAccount, deactivateDirectDebit } from '../../utils/payment.js';
import {
  forgotPasswordMail,
...
import webHooksDataModel from '../../models/beneficiaries/webHooksDataModel.js';
...
export const initDirectDebit = async ({ user, body }) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      initializeDirectDebit(data, (error, body) => {
        if (error) {
          reject(new BadRequestError(error.message));
        }
        const response = JSON.parse(body);
        return resolve({ gateway: response.data.redirect_url });
      });
    } catch (error) {
      return reject(new BadRequestError(error.message));
    }
...
export const debitAccount = async ({ body }) => {
  let organe = await organizationModel.findOne({ email: body.email });
  const data = {
    email: body.email,
    authorization_code: organe.directDebitData.authorization_code,
    amount: 10000,
    currency: NGN
  };
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ChargeUserAccount(data, (error, body) => {
        if (error) {
          reject(new BadRequestError(error.message));
        }
        const response = JSON.parse(body);
...
export const deactivateUserDirectDebit = async ({ user }) => {
  let organe = await organizationModel.findOne({ email: user.email });
  const data = {
    authorization_code: organe.directDebitData.authorization_code
  };
  ...
  return new Promise(async (resolve, reject) => {
    try {
      deactivateDirectDebit(data, (error, body) => {
        if (error) {
          reject(new BadRequestError(error.message));
        }
        const response = JSON.parse(body);
...
export const processPaymentWebHook = async ({ body }) => {
  ...
  switch (body.event) {
    case "direct_debit.authorization.created":
      createdData = await webHooksDataModel.create(body);
      debitData = {
        authorization_code: body.data.authorization_code,
        active: body.data.active,
        last4: body.data.last4,
        channel: body.data.channel,
        card_type: body.data.card_type,
        bank: body.data.bank,
        exp_month: body.data.exp_month,
        exp_year: body.data.exp_year,
        country_code: body.data.country_code,
        brand: body.data.brand,
        reusable: body.data.reusable,
        signature: body.data.signature,
        account_name: body.data.account_name,
        reference: '',
      };
      organe = await organizationModel.findOne({ email: body.data.customer.email });
      organe.directDebitData = debitData;
      await organe.save();
      break;
    case "direct_debit.authorization.active":
      createdData = await webHooksDataModel.create(body);
      organe = await organizationModel.findOne({ email: body.data.customer.email });
      organe.directDebitData.reference = body.data.reference;
      await organe.save();
    case "charge.success":
      createdData = await webHooksDataModel.create(body);
      organe = await organizationModel.findOne({ email: body.data.customer.email });
